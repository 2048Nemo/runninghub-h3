...
name: runninghub-h3
description: >-
  通过 RunningHub 云端运行 MiniMax H3 生成视频（AI 应用与工作流双通道），全链路端到端：
  提交后自动下载成片，抽帧检查是否达标，不达标自动编排重新提交，循环直到抽到目标镜头。
...
- 要端到端闭环出片：提交 → **自动下载** → **抽帧检查是否达标** → **不达标自动编排重试**
  （改提示词/参数/素材，或原样重摇），循环直到抽到想要的镜头。
...
### 闭环：五步只是管道，循环直到抽到想要的镜头

视频生成有随机性，一次出片 ≠ 可用镜头。标准生产循环：

1. **自动下载**：任务 SUCCESS 即落盘 `~/Downloads/runninghub/<项目>/<镜头>.mp4`，
   无需人工守候；本地轮询断了用 `watch --task <id>` 接管，云端任务不受影响。
2. **达标检查**：抽帧检查是否达标——角色一致性、构图与景别、动作与表情、
   时长与分辨率、有无脏画面。具备视觉能力的 Agent 直接自检：
   `ffmpeg -ss <秒> -i 成片.mp4 -frames:v 1 /tmp/check.jpg`，逐帧看。
3. **不达标 → 自动重编排**：定位问题在哪层，就改哪层——
   - 提示词问题（动作/表情/构图不对）→ 改 `263:text` 重提；
   - 参数问题（时长/比例/分辨率/二采倍率）→ 改对应节点参数；
   - 随机性不满意（各要素都对，但「没抽中」）→ 同参数原样重摇；
   - 素材问题（参考视频/图/音频不理想）→ 换素材重传。
4. **重跑 `submit` 即完成一轮**：素材按内容哈希去重上传；`--project/--shot` 撞名自动加
   序号，每一版成片都保留可对比；成功才计费、失败零扣费，重试成本就是 GPU 时间；
   长循环记得 `--max-seconds 3600`。
